    ...
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-10 animate-in fade-in duration-700">
      ...
      <div id="analysis-report-content" className="space-y-12 bg-slate-50">
        ...
        <div className="text-center space-y-2 hidden print:block pt-10">
          ...
          <p className="text-slate-500 font-medium tracking-widest uppercase text-xs">Generated by ContractClarity AI • Grounded in Current Legal Search</p>
        </div>
        ...
        {data.sources && data.sources.length > 0 && (
          <div className="bg-white rounded-[2.5rem] p-10 border border-slate-200 shadow-sm print:rounded-3xl no-print">
            <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center">
              <span className="bg-green-100 text-green-700 w-10 h-10 rounded-xl flex items-center justify-center mr-4 text-sm font-black">Ref</span>
              Verified Legal Context
            </h2>
            <p className="text-slate-500 text-sm mb-6">Our AI cross-referenced this analysis with current legal frameworks and public records. Review the sources below for deeper context:</p>
            <div className="flex flex-wrap gap-3">
              {data.sources.map((source, idx) => (
                <a 
                  ...
                  className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-blue-700 font-bold text-sm hover:bg-blue-50 hover:border-blue-200 transition-all flex items-center space-x-2"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                  <span>{source.title.length > 40 ? source.title.substring(0, 40) + '...' : source.title}</span>
                </a>
              ))}
            </div>
          </div>
        ...
        <div className={`rounded-[2.5rem] p-10 border shadow-sm print:rounded-3xl print:bg-white ${data.summary.overallRiskLevel === RiskLevel.HIGH ? 'bg-orange-50 border-orange-200' : 'bg-blue-50 border-blue-100'}`}>
          ...
          <div className="flex flex-col md:flex-row gap-10 items-start">
            ...
            <div className="flex-1">
              ...
              <p className="mt-4 text-slate-500 text-sm italic">This analysis is powered by Gemini 3 and grounded in real-time legal search results.</p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center pt-10 border-t border-slate-200">
          <div className="max-w-2xl mx-auto space-y-6">
            <p className="text-slate-400 text-xs leading-relaxed uppercase tracking-widest font-bold">
              Important: This is an AI-generated analysis cross-referenced with public search results. 
              It does NOT constitute legal advice. Always review high-stakes contracts with a qualified attorney.
            </p>
